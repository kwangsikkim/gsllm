...
- 목표:
  1) 정형 질문은 안정적으로 처리
  2) 프로젝트 종료 후에도 예외 질문 상당수를 LLM SAFE fallback이 커버
  3) 잘못된 라우팅(Q4로 새는 문제, DETAIL 컬럼 손실, compare 질문 오작동) 보완
  4) 기간형 가격 질문은 기본적으로 "기간 내 행(row) 전체"에서 랭킹

추가 보강점(v4.0.2)
1) "최근 5일 농심 제품 중 평균가 낮은 순으로 8개 보여줘"
   -> 기간 평균 재집계가 아니라 5일 구간의 모든 row 중 avg_price 낮은 8개
   -> batch_date 포함
2) 기간형 generic 가격 질문도 batch_date 포함 + metric 우선 정렬
   - 예: "최근 일주일 싼 상품들 보여줘"
3) 제조사 기간형 TopN 기본 해석을 row-ranking(Q7C)으로 변경
   - 기간 평균/집계 의도가 명시될 때만 Q7B(재집계형) 사용
4) compare 질문은 제조사축 / 몰축 집계 유지
"""
...
PERIOD_AGG_HINT_WORDS = [
    "기간 평균", "구간 평균", "일평균", "평균으로", "평균 기준",
    "묶어서 평균", "집계", "집계해서", "평균 집계", "최근 평균"
]
...
def wants_period_aggregate(q: str) -> bool:
    return any(k in q for k in PERIOD_AGG_HINT_WORDS)
...
def extract_semantic_slots(q: str) -> Dict[str, Any]:
    ...
    slots = {
        ...
        "wants_period_aggregate": wants_period_aggregate(q),
    }
...
def detect_intent(
    ...
    *,
    ...
    has_range = (start_date != end_date) or s["has_range"]
    ...
    if has_product and (s["wants_offer"] or s["wants_detail"] or s["prefers_raw"]):
        return "Q1_DETAIL"

    if has_mall and s["not_cheapest"] and has_range and (s["wants_date_in_rows"] or s["wants_trend"] or s["wants_group_by_mall"]):
    ...
    if has_mall and s["wants_topn"] and s["has_metric"] and (not multi_mall):
        return "Q7"
...
def upsert_price_compare(col_sum, df_pc: pd.DataFrame, batch_size: int = 500, version: str = "v4_0_2"):
    ids: List[str] = []
    docs: List[str] = []
...
def upsert_reviews_digest(col_rev, df_data: pd.DataFrame, batch_size: int = 500, version: str = "v4_0_2"):
...
def q5_range_summary(
    ...
    for d in dates:
        one = q5_mall_not_cheapest(
            cur[cur["batch_date"] == d].copy(),
            batch_date=d,
            target_mall=target_mall,
            manufacturer=manufacturer,
        ...
        rows.append({
            "batch_date": d,
            "manufacturer": manufacturer if manufacturer else "ALL",
            "mall_name": target_mall,
            "violations_count": int(one["count"]),
            "total_products": int(one["total_products"]),
        })
    ...
    return out.sort_values("batch_date", ascending=True).reset_index(drop=True)
...
def q7c_manufacturer_range_topn_with_date(
    ...
    sort_cols = ["value", "batch_date", "mall_name", "product_key"]
    sort_asc = [ascending, True, True, True]
    cur = cur.sort_values(sort_cols, ascending=sort_asc).head(int(max(1, n))).reset_index(drop=True)
    return cur
...
def build_llm_intent_context(today: str, default_start: str) -> str:
    schema_line = (
        '{'
        '"intent": "Q1" | "Q1_DETAIL" | "Q4" | "Q5" | "Q5_RANGE_SUMMARY" | "Q6" | "Q7" | "Q8_TREND" | '
...
def validate_llm_intent(intent: str, *, mall: Optional[str], pkey: Optional[str], manu: Optional[str]) -> bool:
    if intent in ["A1_VIOL_DETAIL", "A1_VIOL_TREND", "Q5", "Q5_RANGE_SUMMARY", "Q6", "Q7", "Q9_MALL_SUMMARY_TABLE"] and not mall:
        return False
...
def llm_classify_intent(
    ...
    *,
    ...
    allow = {
        "Q1", "Q1_DETAIL", "Q4", "Q5", "Q5_RANGE_SUMMARY", "Q6", "Q7", "Q8_TREND",
        "Q7B_MANU_RANGE_TOPN", "Q7C_MANU_RANGE_TOPN_WITH_DATE", "Q7D_MANU_RANGE_TOPN_BY_MALL",
        "Q9_MALL_SUMMARY_TABLE", "A1_VIOL_TREND", "A1_VIOL_DETAIL", "UNKNOWN"
    }
...
def build_llm_plan_context(
    ...
    return (
        ...
        "- For manufacturer range ranking, default to row-level ranking with batch_date included unless question explicitly asks for period-average aggregation.\n"
...
def build_rule_based_miniplan(
    ...
    *,
    ...
    if source == "df_pc_all":
        ...
        if start_date != end_date and not pkey and not has_compare_words(q):
            if manu and not wants_period_aggregate(q) and not wants_group_by_mall(q):
                return {
                    "source": "df_pc_all",
                    ...
                    "groupby": [],
                    "aggregations": [],
                    "sort": [{"col": metric, "dir": sort_dir}, {"col": "batch_date", "dir": "asc"}],
                    "limit": limit,
                }

            if not mall and not manu:
                return {
                    "source": "df_pc_all",
                    "select": ["batch_date", "manufacturer", "mall_name", "product_key", "product_name", metric],
                    "filters": filters,
    ...
    *,
    ...
    if has_generic_price_bucket_words(q):
        ...
        if start_date != end_date and not mall and not manu and not pkey:
            plan["select"] = ["batch_date", "manufacturer", "mall_name", "product_key", "product_name", metric]
            plan["sort"] = [{"col": metric, "dir": "asc" if parse_sort_direction(q, metric) else "desc"}, {"col": "batch_date", "dir": "asc"}]
        else:
            plan["select"] = ["manufacturer", "mall_name", "product_key", "product_name", metric]
            plan["sort"] = [{"col": metric, "dir": "asc" if parse_sort_direction(q, metric) else "desc"}]
        plan["groupby"] = []
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Snacks POC CLI Router v4.0.2")
...
def main():
    ...
    print("=== Snacks POC CLI Router v4.0.2 ===")
    ...
    if args.enable_chroma:
        try:
            ...
            upsert_price_compare(col_sum, df_pc_all, version="v4_0_2")
            if not args.disable_reviews_digest:
                upsert_reviews_digest(col_rev, df_data_all, version="v4_0_2")
            print("Chroma 업서트 완료")
        except Exception as e:
    ...
    while True:
        ...
        if intent == "Q5_RANGE_SUMMARY":
            ...
            print(f"\n[정형결과] {mall}보다 싼 곳이 있는 상품 날짜별 요약 ({start_date} ~ {end_date}{scope})")
