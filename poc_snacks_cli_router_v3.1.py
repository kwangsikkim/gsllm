...
"""
poc_snacks_cli_router_v3.1.py

v3.2.6.1-full
- 기반: v3.2.6-full
- 목적:
  1) 정형 intent를 슬롯 기반으로 강화
  2) LLM intent fallback을 '표현 복원' 용도로 제한
  3) 위반/비교 계열 질의는 SAFE plan fallback을 차단
  4) LLM SAFE fallback 디버깅을 위해 RAW PLAN / NORMALIZED PLAN 출력 지원
...
def parse_dates_list(question: str, default_date: str) -> List[str]:
    ...
    if dates:
        inferred_year = int(dates[0].split("-", 1)[0])
    else:
        try:
            inferred_year = int(str(default_date)[:4])
        except Exception:
    ...
    stripped = re.sub(r"\d{4}\s*년\s*\d{1,2}\s*월\s*\d{1,2}\s*일", " ", q)
    stripped = re.sub(r"\d{4}[-/]\d{1,2}[-/]\d{1,2}", " ", stripped)

    if inferred_year is not None:
        for mm in re.finditer(r"(\d{1,2})\s*월\s*(\d{1,2})\s*일", stripped):
            mo, d = int(mm.group(1)), int(mm.group(2))
            dates.append(f"{inferred_year:04d}-{mo:02d}-{d:02d}")

        for mm in re.finditer(r"\b(\d{1,2})\s*[-/]\s*(\d{1,2})\b", stripped):
            mo, d = int(mm.group(1)), int(mm.group(2))
            dates.append(f"{inferred_year:04d}-{mo:02d}-{d:02d}")
...
def parse_date_range(question: str, default_end: str, default_start: str) -> Tuple[str, str]:
    q0 = question.strip()
    q = normalize_range_separators(q0)
    ...
    has_range_signal = any(k in q for k in ["~", "-", "부터", "까지", "최근", "지난"])
    has_single_signal = (
        any(k in q for k in ["오늘", "금일", "어제", "그제", "그저께", "내일", "모레"])
        or bool(re.search(r"(\d{1,3})\s*일\s*(전|이전|앞|후|뒤)", q))
        or bool(re.search(r"\d{4}\s*년\s*\d{1,2}\s*월\s*\d{1,2}\s*일", q))
        or bool(re.search(r"\d{4}[-/]\d{1,2}[-/]\d{1,2}", q))
        or bool(re.search(r"\b(\d{1,2})\s*[-/]\s*(\d{1,2})\b", q))
        or bool(re.search(r"(\d{1,2})\s*월\s*(\d{1,2})\s*일", q))
    )
    if has_single_signal and (not has_range_signal):
        single = parse_batch_date(q, default_date=default_end)
        return max(single, default_start), min(single, default_end)

    m = re.search(
        r"(\d{4})\s*년\s*(\d{1,2})\s*월\s*(\d{1,2})\s*일\s*-\s*(\d{1,2})\s*월\s*(\d{1,2})\s*일",
        q
    )
    if m:
        y = int(m.group(1))
        ...
        s = f"{y:04d}-{m1:02d}-{d1:02d}"
        e = f"{y:04d}-{m2:02d}-{d2:02d}"
        if s > e:
            s, e = e, s
        return max(s, default_start), min(e, default_end)
    ...
    if m2:
        y1, mo1, d1 = int(m2.group(1)), int(m2.group(2)), int(m2.group(3))
        y2, mo2, d2 = int(m2.group(4)), int(m2.group(5)), int(m2.group(6))
        s = f"{y1:04d}-{mo1:02d}-{d1:02d}"
        e = f"{y2:04d}-{mo2:02d}-{d2:02d}"
        if s > e:
            s, e = e, s
        return max(s, default_start), min(e, default_end)
    ...
    if any(k in q for k in ["최근 일주일", "최근 1주일", "지난 일주일", "지난 1주일"]):
        n = 7
        end = default_end
        start = (datetime.strptime(end, "%Y-%m-%d") - timedelta(days=n - 1)).strftime("%Y-%m-%d")
        if start < default_start:
    ...
    if "최근" in q and any(k in q for k in ["한달", "한 달", "1달", "1개월", "한개월", "한 개월"]):
        n = 30
        end = default_end
        start = (datetime.strptime(end, "%Y-%m-%d") - timedelta(days=n - 1)).strftime("%Y-%m-%d")
        if start < default_start:
...
def resolve_my_manufacturer(question: str, manufacturers: List[str], fallback_my: str = MY_MANUFACTURER) -> Optional[str]:
    q = question.strip()
    if has_my_words(q):
        if fallback_my in manufacturers:
            return fallback_my
        return fallback_my
    return None
...
def wants_price_jump(q: str) -> bool:
    q = q.strip()
    return any(
        k in q
        for k in [
            "급등", "폭등", "많이 올랐", "많이 올라", "상승", "상승폭", "급등폭",
            "가격이 많이 오른", "가격이 많이 올랐", "가격 올랐", "가격 올라", "추세가 급",
        ]
    )
...
def parse_sort_direction(question: str, metric: str) -> bool:
    q = question.strip()
    if any(k in q for k in ["높", "비싼", "최고", "큰", "비싸게", "가장 비싼"]):
        return False
    if metric == "max_price":
        return False
    return True
...
TOP_WORDS_EXT = [
    "상위", "하위", "top", "TOP", "탑", "베스트", "랭킹", "순위"
]

Q5_NEG_WORDS = ["아닌", "아니다", "아닙", "아니", "못", "놓친", "실패", "놓쳤"]
...
def _has_topn_semantics(q: str) -> bool:
    return any(w in q for w in TOP_WORDS_EXT)
...
def load_excel_one(xlsx_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if sheet_data not in sheets:
        if "DATA" in sheets:
            ...
            raise ValueError(f"DATA 시트를 찾을 수 없습니다. file={xlsx_path} sheets={sorted(sheets)}")

    if sheet_pc not in sheets:
        if "PRICE_COMPARE" in sheets:
            sheet_pc = "PRICE_COMPARE"
        else:
            raise ValueError(f"PRICE_COMPARE 시트를 찾을 수 없습니다. file={xlsx_path} sheets={sorted(sheets)}")
    ...
    for col in required_data:
        if col not in df_data.columns:
            raise ValueError(f"DATA 시트에 필수 컬럼이 없습니다: {col} (file={xlsx_path}, sheet={sheet_data})")
    for col in required_pc:
        if col not in df_pc.columns:
            raise ValueError(f"PRICE_COMPARE 시트에 필수 컬럼이 없습니다: {col} (file={xlsx_path}, sheet={sheet_pc})")
...
def upsert_price_compare(col_sum, df_pc: pd.DataFrame, batch_size: int = 500, version: str = "v3_2_6_1_full"):
    ids: List[str] = []
    docs: List[str] = []
...
def upsert_reviews_digest(col_rev, df_data: pd.DataFrame, batch_size: int = 500, version: str = "v3_2_6_1_full"):
...
def q1_product_all_offers(df_data: pd.DataFrame, product_key: str, batch_date: str) -> Optional[pd.DataFrame]:
    ...
    if not sort_cols:
        sort_cols = ["date"] if "date" in cur.columns else []

    out = cur[cols].sort_values(sort_cols, ascending=True).reset_index(drop=True) if sort_cols else cur[cols].reset_index(drop=True)
    return out
...
def q5_mall_not_cheapest(
    ...
    tgt = cur[cur["mall_name"] == target_mall][
        ["manufacturer", "product_key", "product_name", "min_price"]
    ].copy()
    ...
    others = cur[cur["mall_name"] != target_mall][
        ["manufacturer", "product_key", "product_name", "mall_name", "min_price"]
    ].copy()
    others = others.rename(columns={
        "mall_name": "cheaper_mall",
        "min_price": "cheaper_price",
    })

    out = others.merge(
        tgt[["product_key", "target_mall_price"]],
        on="product_key",
        how="inner",
    )
    ...
    sort_cols = ["product_key", "cheaper_price", "cheaper_mall"]
    ...
    violated_products = int(out["product_key"].nunique())
...
def q7_mall_metric_topn(
    ...
    out = cur[["manufacturer", "product_key", "product_name", metric]] \
...
def q8_product_trend(
    ...
    if len(cur) == 0:
        return None
    out = cur[["batch_date", "manufacturer", "mall_name", "min_price", "avg_price", "max_price"]] \
        .sort_values(["batch_date", "mall_name"], ascending=[True, True]) \
...
def a1_violation_trend(
    ...
    merged["has_target"] = pd.notna(merged["target_mall_price"])
    merged = merged[merged["has_target"]].copy()
    if len(merged) == 0:
        return None
    ...
    grp["sum_diff"] = grp["sum_diff"].astype(float)
    grp["avg_diff"] = grp["avg_diff"].astype(float)

    grp = grp.sort_values("batch_date", ascending=True).reset_index(drop=True)
    return grp
...
def a1_violation_detail(
    ...
    tgt = cur[cur["mall_name"] == target_mall][
        ["batch_date", "product_key", "product_name", "min_price"]
    ].copy()
    tgt = tgt.rename(columns={"min_price": "target_mall_price"})

    if len(tgt) == 0:
        return pd.DataFrame(columns=[
            "batch_date", "product_key", "product_name",
            "target_mall_price", "cheaper_mall", "cheaper_price", "diff"
        ])

    others = cur[cur["mall_name"] != target_mall][
        ["batch_date", "product_key", "product_name", "mall_name", "min_price"]
    ].copy()
    others = others.rename(columns={
        "mall_name": "cheaper_mall",
        "min_price": "cheaper_price",
    ...
    out = others.merge(
        tgt[["batch_date", "product_key", "target_mall_price"]],
        on=["batch_date", "product_key"],
        how="inner",
    )
    ...
    out = out[[
        ...
        ["batch_date", "product_key", "diff", "cheaper_price"],
...
# ============================================================
# LLM fallback (SAFE): intent-only + plan
# ============================================================
LLM = None
...
def build_llm_intent_context(today: str, default_start: str) -> str:
    schema_line = (
        '{'
        '"intent": "Q1" | "Q1_DETAIL" | "Q4" | "Q5" | "Q6" | "Q7" | "Q8_TREND" | "A1_VIOL_TREND" | "A1_VIOL_DETAIL" | "UNKNOWN", '
    ...
    return (
        ...
        f"{schema_line}\n\n"
        "Rules:\n"
        "- violation/위반/보상/페널티 + mall => A1_*\n"
        "- detail/list/internal detail words (상세/내역/목록/리스트/전부/전체) => DETAIL preference\n"
        "- trend words (추이/트렌드/변화/변동/흐름/추세/그래프/차트/일별/날짜별/기간별) => TREND preference\n"
        "- offer/raw/list => Q1_DETAIL\n"
        "- product price/best/시세/얼마 => Q1\n"
        "- mall not cheapest => Q5\n"
        "- mall cheapest only => Q6\n"
        "- mall topN min/avg/max => Q7\n"
        "- product trend => Q8_TREND\n"
        "- shipping issue (배송 + 불만/문제/이슈 등) => Q4\n"
        "- If unsure, return UNKNOWN.\n"
    )


def validate_llm_intent(intent: str, *, mall: Optional[str], pkey: Optional[str]) -> bool:
    if intent in ["A1_VIOL_DETAIL", "A1_VIOL_TREND", "Q5", "Q6", "Q7"] and not mall:
        return False
...
def llm_classify_intent(
    ...
    *,
    ...
    allow = {"Q1", "Q1_DETAIL", "Q4", "Q5", "Q6", "Q7", "Q8_TREND", "A1_VIOL_TREND", "A1_VIOL_DETAIL", "UNKNOWN"}
    if intent not in allow:
        return "UNKNOWN"

    signals = obj.get("signals") or {}
    has_detail = bool(signals.get("has_detail", False))
    has_trend = bool(signals.get("has_trend", False))

    if intent == "A1_VIOL_TREND" and (not has_trend):
        return "UNKNOWN"
    if intent == "A1_VIOL_DETAIL" and has_trend and (not has_detail):
        return "UNKNOWN"

    if not validate_llm_intent(intent, mall=mall, pkey=pkey):
        return "UNKNOWN"
...
def print_normalized_plan(plan: PandasPlan):
    try:
        ...
        print(f"[NORMALIZED PLAN 출력 실패] {repr(e)}")


def build_llm_plan_context(df_pc_all: pd.DataFrame, df_data_all: pd.DataFrame, today: str) -> str:
    ...
    return (
        "You convert a Korean user question into a STRICT JSON 'PandasPlan' to query two in-memory pandas DataFrames.\n"
        "Output ONLY one JSON object. No explanation. No markdown.\n\n"
        f"today(batch_date) is: {today}\n\n"
        "Available sources:\n"
        "- df_pc_all: daily summary per product_key x mall_name x batch_date\n"
        "- df_data_all: raw offers with rank/price/comments\n\n"
        "PandasPlan schema:\n"
        "{\n"
        '  "source": "df_pc_all" | "df_data_all",\n'
        '  "select": ["col1","col2",...],\n'
        '  "filters": [\n'
        '     {"col":"batch_date","op":"between","start":"YYYY-MM-DD","end":"YYYY-MM-DD"},\n'
        '     {"col":"batch_date","op":"=","value":"YYYY-MM-DD"},\n'
        '     {"col":"manufacturer","op":"=","value":"해태제과"},\n'
        '     {"col":"product_key","op":"=","value":"H036"},\n'
        '     {"col":"comments_top5","op":"contains","value":"배송"}\n'
        "  ],\n"
        '  "groupby": ["mall_name", ...],\n'
        '  "aggregations": [{"col":"min_price","func":"mean","as":"avg_min_price"}, ...],\n'
        '  "sort": [{"col":"avg_min_price","dir":"asc"}],\n'
        f'  "limit": 1..{HARD_LIMIT_ROWS}\n'
        "}\n\n"
        "Rules:\n"
        "- Use only columns that exist in the selected source.\n"
        "- Always include LIMIT <= HARD_LIMIT.\n"
        "- If user asks offers/list/details, prefer df_data_all.\n"
        "- If user asks daily price/min/avg/max/topN/trend, prefer df_pc_all.\n"
        "- Keep select small (< 12 columns) unless requested.\n"
        "- DO NOT invent manufacturer/product_key.\n"
        "- DO NOT attempt policy/violation/not-cheapest/comparison business logic.\n"
        "- Only handle plain filtering/grouping/sorting questions.\n\n"
...
def llm_generate_plan(
    ...
    jtxt = _extract_json_object(raw)
    if not jtxt:
        raise ValueError(f"LLM output did not contain JSON object. raw={raw[:240]!r}")
...
def validate_and_normalize_plan(plan: Dict[str, Any], df_pc_all: pd.DataFrame, df_data_all: pd.DataFrame) -> PandasPlan:
    ...
    limit = _clamp_int(plan.get("limit"), 1, HARD_LIMIT_ROWS, 200)
...
def execute_plan(plan: PandasPlan, df_pc_all: pd.DataFrame, df_data_all: pd.DataFrame) -> pd.DataFrame:
    ...
    if plan.groupby and plan.aggregations:
        ...
        grp.columns = [f"{c0}_{c1}" for (c0, c1) in grp.columns.to_list()]
        out = grp.reset_index()
    else:
        out = cur
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Snacks POC CLI Router v3.2.6.1-full")
    ...
    p.add_argument("--print-plan", action="store_true", help="print RAW/NORMALIZED plan in plan fallback")
    p.add_argument("--disable-llm-plan", action="store_true", help="disable PandasPlan fallback (intent-only only)")
    return p
...
def main():
    ...
    print("=== Snacks POC CLI Router v3.2.6.1-full ===")
    ...
    if args.enable_chroma:
        try:
            ...
            upsert_price_compare(col_sum, df_pc_all, version="v3_2_6_1_full")
            if not args.disable_reviews_digest:
                upsert_reviews_digest(col_rev, df_data_all, version="v3_2_6_1_full")
            print("Chroma 업서트 완료")
        except Exception as e:
    ...
    llm_plan_ctx = build_llm_plan_context(df_pc_all, df_data_all, today=today) if llm_plan_enabled else None
    ...
    while True:
        ...
        print("[DEBUG:slots]", extract_semantic_slots(q))
        ...
        if intent in ["A1_VIOL_TREND", "A1_VIOL_DETAIL"] and not manu:
            manu = MY_MANUFACTURER if MY_MANUFACTURER in catalog["manufacturers"] else MY_MANUFACTURER

        if intent in ["Q1", "Q1_DETAIL", "Q8_TREND"] and not pkey:
            print("제품을 인식 못했어요. (엑셀에 존재하는 제품코드/제품명이 질문에 포함되어야 함)\n")
            continue
        if intent in ["Q5", "Q6", "Q7", "A1_VIOL_TREND", "A1_VIOL_DETAIL"] and not mall:
        ...
        if intent == "Q1_DETAIL":
            run_dates = dates_list if dates_list else [batch_date]
            offers = q1_product_all_offers(df_data_all, pkey, run_dates[0])
            if offers is None:
                print(f"\n[정형결과] {pkey} {run_dates[0]} 오퍼(원본) 데이터를 못 찾았어요.\n")
                continue
            print(f"\n[정형결과] {pkey} {run_dates[0]} 오퍼 목록 {len(offers)}건")
            print_result_any(offers, output_dir=args.output_dir, prefix=f"Q1_DETAIL_{pkey}_{run_dates[0]}")
            print()
            continue
        ...
        if intent == "Q6":
            ...
            scope = f"(제조사={manu}) " if manu else ""
            print(
                f"\n[정형결과] {batch_date} {scope}'{mall}'이(가) 최저가인 제품: "
                f"{out['count']}개 / 전체 {out['total_products']}개"
            )
            table = out["table"]
            if table.empty:
            ...
            ensure_dir(args.output_dir)
            mtag = manu if manu else "ALL"
            path = os.path.join(args.output_dir, f"Q6_cheapest_{mall}_{mtag}_{batch_date}.csv")
            table.to_csv(path, index=False, encoding="utf-8-sig")
            print(f"\n[저장] CSV: {path}\n")
            continue
        ...
        if intent == "Q7":
            ...
            ts = now_ts()
            mtag = manu if manu else "ALL"
            path = os.path.join(args.output_dir, f"Q7_{mall}_{metric}_{direction.replace(' ', '')}_{mtag}_{batch_date}_{ts}.csv")
            out["table"].to_csv(path, index=False, encoding="utf-8-sig")
            print(f"\n[저장] CSV: {path}\n")
            continue
        ...
        if not llm_plan_enabled:
            ...
            continue
        if not llm_plan_ctx:
            llm_plan_ctx = build_llm_plan_context(df_pc_all, df_data_all, today=today)
        ...
        try:
            ...
            plan = validate_and_normalize_plan(raw_plan, df_pc_all=df_pc_all, df_data_all=df_data_all)
