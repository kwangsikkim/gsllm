...
class HybridQASystem:
    ...
    # ------------------------------------------------------------------
    # 이미지 / 테이블 추출 (의미 기반 + 페이지 + 섹션 + 쿼리 키워드 필터)
    # ------------------------------------------------------------------
    def extract_images_and_tables(
        ...
        - 리랭커 상위 결과들 중에서 type == "text" 인 것의 (pdf_name, page, section)를 모은다.
        - pdf_name별로 section 빈도를 세어서 상위 section만 허용 섹션으로 선택한다.
        - 쿼리에서 의미 있는 키워드를 뽑아서(media_text / local_text / 문서 텍스트에 최소 1개 이상 포함되어야 함)
        - manual_media_embeddings에서 media_text 기준으로 query와 가장 비슷한 media를 가져와서,
          (pdf_name, page)가 text_pages(±1 페이지) 안에 있고,
          section도 허용 섹션에 포함되며,
          media_text 등에서 쿼리 키워드가 최소 1개 이상 등장하는 media만 남긴다.
        ...
        for result in results:
            ...
            if metadata.get("type") == "text":
                ...
                if section:
                    if pdf_name not in section_counts_by_pdf:
                        section_counts_by_pdf[pdf_name] = {}
                    section_counts_by_pdf[pdf_name][section] = (
                        section_counts_by_pdf[pdf_name].get(section, 0) + 1
                    )

        # 1-1) 각 PDF의 페이지에 대해 연속된 페이지 추가 (±1)
        ...
        # 1-2) 각 PDF별로 "허용 섹션" 결정 (상위 1~2개)
        allowed_sections_by_pdf: Dict[Optional[str], Set[str]] = {}
        for pdf_name, counts in section_counts_by_pdf.items():
            if not counts:
                continue
            sorted_sections = sorted(
                counts.items(), key=lambda x: x[1], reverse=True
            )
            top_sections = [sec for sec, _ in sorted_sections[:2]]
            allowed_sections_by_pdf[pdf_name] = set(top_sections)

        # 1-3) 쿼리에서 의미 있는 키워드 추출 (media_text / local_text 필터용)
        raw_q = re.sub(r"[^0-9a-zA-Z가-힣 ]", " ", query)
        pieces = [p for p in raw_q.split() if len(p) >= 2]
        stopwords = {
            "에",
            "대한",
            "대해",
        ...
        query_terms = [p for p in pieces if p not in stopwords]
        # 중복 제거 유지
        query_terms = list(dict.fromkeys(query_terms))

        # 2) media 컬렉션이 있을 때: 의미 기반 + 페이지 + 섹션 + 키워드 필터
        if self.media_collection is not None and text_pages:
            ...
            for mr in media_results:
                ...
                # 섹션 필터: 해당 pdf에서 많이 등장한 섹션에 속하는 media만 허용
                allowed_sections = allowed_sections_by_pdf.get(pdf_name)
                m_section = md.get("section")
                if allowed_sections:
                    if m_section and m_section not in allowed_sections:
                        # 다른 섹션(예: 바로 위 메타버스 섹션)에 속하는 media는 스킵
                        continue
                ...
                media_text = mr.get("document", "") or ""
                # 쿼리 키워드와 media_text / 섹션 / 서브섹션 매칭
                if query_terms:
                    searchable = " ".join(
                        [
                            media_text,
                            str(md.get("section") or ""),
                            str(md.get("subsection") or ""),
                        ]
                    )
                    searchable_lower = searchable.lower()
                    matched = any(term.lower() in searchable_lower for term in query_terms)
                    if not matched:
                        # 쿼리 핵심 키워드와 전혀 안 겹치면 버림
                        continue

                summary = re.sub(r"\s+", " ", media_text).strip()
        ...
        # 3) fallback: media 컬렉션이 없거나, 위 과정에서 media가 하나도 못 뽑힌 경우
        if not related_images and not related_tables:
            # 3-1) 검색 결과에 직접 등장한 이미지/테이블 추가
            for result in results:
                metadata = result.get("metadata", {})
                ...
                # 쿼리 키워드와 해당 문서 텍스트(result["document"]) 매칭
                if query_terms:
                    doc_text = result.get("document", "") or ""
                    doc_lower = doc_text.lower()
                    if doc_text and not any(
                        term.lower() in doc_lower for term in query_terms
                    ):
                        continue
            ...
            # 3-2) 텍스트가 있는 페이지에서 연관 이미지/테이블 추가
            for pdf_name, page in text_pages:
                ...
                for el in elements:
                    ...
                    # page_elements에서도 사이드바 이미지 제외
                    if etype == "image" and el.get("is_sidebar", False):
                        continue

                    # 쿼리 키워드와 media_text / local_text 매칭
                    if query_terms:
                        searchable = " ".join(
                            [
                                str(el.get("media_text") or ""),
                                str(el.get("local_text") or ""),
                            ]
                        )
                        searchable_lower = searchable.lower()
                        if searchable and not any(
                            term.lower() in searchable_lower for term in query_terms
                        ):
                            continue
    ...
    def _clean_answer(self, answer: str) -> str:
        ...
        for line in lines:
            # 참고 문서 섹션 시작 감지
            if line.strip().startswith("참고 문서:") or line.strip().startswith(
                "참고 문서"
            ):
                skip_rest = True
                break
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--server":
        ...
        async def ask_question(request: QuestionRequest):
            ...
            try:
                ...
                logger.info(
                    f"Answer generated successfully. Sources: {result.get('num_sources', 0)}"
                )
        ...
        async def health():
            """헬스 체크 엔드포인트"""
            return {"status": "ok", "qa_system_loaded": qa_system is not None}
