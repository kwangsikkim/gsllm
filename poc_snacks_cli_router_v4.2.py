...
"""
poc_snacks_cli_router_v4.py

v4.2.3
- v4.2.2 기반 개선

이번 추가 수정:
1) 단일 날짜(예: 2026-03-01)가 기간형으로 오인되지 않도록 _has_range_semantics()에서 '-' 제거
2) detect_intent()에서 compare 단어만으로 즉시 UNKNOWN 처리하던 로직 제거
   - 정형 처리 가능한 비교 질의는 먼저 정형 intent로 라우팅
3) should_block_plan_fallback()를 보수적으로 수정
   - 위반/최저가 아님/타겟몰보다 싼 곳 존재 같은 비즈니스 비교 질의는 plan fallback 차단
4) Q5 / Q5_RANGE_* / A1 merge key 강화
   - manufacturer/product_key(+batch_date) 기준으로 더 안전하게 매칭
5) parse_dates_list()가 상대일/절대일 혼합 질의에서도 둘 다 수집하도록 수정
6) q1_product_best() 동률 최저가 지원
   - 복수 최저가 몰이면 모두 표시하고 대표 best_mall은 쉼표로 연결
"""
...
def parse_dates_list(question: str, default_date: str) -> List[str]:
    ...
    for kw, delta in rel_map:
        idx = q.find(kw)
        if idx >= 0:
            dates.append((idx, shift_date_ymd(default_date, delta)))

    for mm in re.finditer(r"(\d{1,3})\s*일\s*(전|이전|앞)", q):
        n = max(1, min(int(mm.group(1)), 365))
        dates.append((mm.start(), shift_date_ymd(default_date, -n)))
...
def parse_date_range(question: str, default_end: str, default_start: str) -> Tuple[str, str]:
    ...
    if any(k in q for k in ["최근 일주일", "최근 1주일", "지난 일주일", "최근 일주일동안", "최근 일주일 동안"]):
        start = (dt_end - timedelta(days=6)).strftime("%Y-%m-%d")
        return max(start, default_start), default_end
...
def parse_metric_kor(question: str) -> str:
    ...
    if has_compare_words(q):
        if any(k in q for k in ["더 비싼", "비교해서 더 비싼", "비싼 쪽", "높은 쪽"]):
            return "avg_price"
        if any(k in q for k in ["더 싼", "싼 쪽", "낮은 쪽"]):
            return "avg_price"
        return "avg_price"

    if any(k in q for k in ["싼 것들", "저렴한 것들", "싼 상품", "저렴한 상품", "싼 애들"]):
        return "min_price"

    if any(k in q for k in ["비싼 것들", "높은 것들", "비싼 상품", "높은 상품", "비싼 애들"]):
        return "max_price"
...
def _is_cheapest_only_semantic(q: str) -> bool:
    ...
    return (
        ...
        and ("아닌" not in q)
        and ("놓친" not in q)
        and ("보다" not in q or "더 싼 곳" not in q)
...
def infer_time_mode(q: str, start_date: str, end_date: str, slots: Dict[str, Any]) -> str:
    if slots["wants_trend"]:
        return "trend"
    if start_date != end_date and slots["has_metric"]:
...
def extract_semantic_slots(q: str) -> Dict[str, Any]:
    ...
    slots["view_source"] = infer_view_source(q)
    return slots
...
def detect_intent(
    ...
    *,
    ...
    has_range = (start_date != end_date)

    if has_mall and s["has_violation"]:
        if s["wants_trend"] or has_range:
            return "A1_VIOL_TREND"
        return "A1_VIOL_DETAIL"

    if has_product and s["wants_price"] and not (s["wants_offer"] or s["prefers_raw"]):
        if s["wants_trend"]:
            return "Q8_TREND"
        return "Q1"

    if has_product and (s["wants_offer"] or s["prefers_raw"]):
        return "Q1_DETAIL"

    if has_product and s["wants_detail"] and not s["wants_price"]:
        return "Q1_DETAIL"

    if has_mall and s["not_cheapest"] and has_range:
        if s["wants_detail"] or s["wants_all_rows"] or any(k in q for k in ["상품", "제품", "목록", "리스트", "전부", "전체", "상세"]):
            return "Q5_RANGE_DETAIL"
        if s["wants_date_in_rows"] or s["wants_trend"] or s["wants_group_by_mall"]:
            return "Q5_RANGE_SUMMARY"
        return "Q5_RANGE_DETAIL"
    ...
    if has_product and (s["wants_price"] or not s["wants_detail"]):
        return "Q1"

    if s["wants_shipping_issue"] and not s["has_generic_price_bucket_words"] and not s["has_compare_words"]:
        return "Q4"

    if has_manu and s["has_metric"] and has_range and (not has_mall) and (not has_product) and (not multi_manu):
    ...
    if has_mall and s["has_metric"] and (s["wants_topn"] or s["has_generic_price_bucket_words"]) and (not has_product) and (not multi_mall):
        if has_range:
            return "UNKNOWN"
        return "Q7"

    if has_mall and s["prefers_summary"] and (not has_product) and (not multi_mall):
        return "Q9_MALL_SUMMARY_TABLE"

    if s["has_compare_words"]:
        return "UNKNOWN"
...
def upsert_price_compare(col_sum, df_pc: pd.DataFrame, batch_size: int = 500, version: str = "v4_2_3"):
    ids: List[str] = []
    docs: List[str] = []
...
def upsert_reviews_digest(col_rev, df_data: pd.DataFrame, batch_size: int = 500, version: str = "v4_2_3"):
...
def q1_product_best(df_pc: pd.DataFrame, product_key: str, batch_date: str):
    ...
    best_price = cur["min_price"].min()
    best_rows = cur[cur["min_price"] == best_price].copy()
    best_rows = best_rows.sort_values(["mall_name", "manufacturer"], ascending=[True, True]).reset_index(drop=True)

    table = cur[["manufacturer", "mall_name", "min_price", "avg_price", "max_price"]].sort_values(
        ["min_price", "mall_name"], ascending=[True, True]
    ).reset_index(drop=True)

    return {
        "manufacturer": str(best_rows.iloc[0].get("manufacturer", "")),
        "best_mall": ", ".join(best_rows["mall_name"].astype(str).tolist()),
        "best_price": int(best_price),
        "best_malls_count": int(len(best_rows)),
        "table": table,
    }
...
def q5_mall_not_cheapest(
    ...
    out = others.merge(
        tgt[["manufacturer", "product_key", "product_name", "target_mall_price"]],
        on=["manufacturer", "product_key", "product_name"],
        how="inner",
    )
    ...
    out = out.sort_values(["manufacturer", "product_key", "cheaper_price", "cheaper_mall"], ascending=True).reset_index(drop=True)
...
def q5_range_summary(
    ...
    for d in dates:
        ...
        tgt = day[day["mall_name"] == target_mall][["manufacturer", "product_key", "product_name", "min_price"]].copy()
        tgt = tgt.rename(columns={"min_price": "target_mall_price"})

        if len(tgt) == 0:
            continue

        others = day[day["mall_name"] != target_mall][["manufacturer", "product_key", "product_name", "mall_name", "min_price"]].copy()
        others = others.rename(columns={"mall_name": "cheaper_mall", "min_price": "cheaper_price"})
...
def q5_range_detail(
    ...
    out = others.merge(
        tgt[["batch_date", "manufacturer", "product_key", "product_name", "target_mall_price"]],
        on=["batch_date", "manufacturer", "product_key", "product_name"],
        how="inner",
    )
    out = out[out["cheaper_price"] < out["target_mall_price"]].copy()
    if len(out) == 0:
    ...
    out = out[keep].sort_values(
        ["batch_date", "manufacturer", "product_key", "cheaper_price", "cheaper_mall"],
...
def q6_mall_is_cheapest(df_pc: pd.DataFrame, batch_date: str, target_mall: str, manufacturer: Optional[str] = None):
    ...
    min_per_product = cur.groupby(["manufacturer", "product_key"])["min_price"].transform("min")
    ...
    ).sort_values(["manufacturer", "product_key", "cheapest_mall"], ascending=True).reset_index(drop=True)
...
def q7_mall_metric_topn(
    ...
    out = cur[["manufacturer", "mall_name", "product_key", "product_name", metric]].sort_values(
        [metric, "product_key"], ascending=[bool(ascending), True]
...
def q7b_manufacturer_range_topn(
    ...
    grp = grp.sort_values(["value", "mall_name", "product_key"], ascending=[ascending, True, True]).head(int(max(1, n))).reset_index(drop=True)
    return grp
...
def q7d_manufacturer_range_topn_by_mall(
    ...
    for mall_name, g in grp.groupby("mall_name", dropna=False):
        g2 = g.sort_values(["value", "product_key"], ascending=[ascending, True]).head(int(max(1, n_per_mall))).reset_index(drop=True)
...
def a1_violation_trend(
    ...
    gmin = cur.groupby(["batch_date", "manufacturer", "product_key"], dropna=False)["min_price"].min().reset_index()
    gmin = gmin.rename(columns={"min_price": "global_min_price"})

    tgt = cur[cur["mall_name"] == target_mall][["batch_date", "manufacturer", "product_key", "min_price"]].copy()
    tgt = tgt.rename(columns={"min_price": "target_mall_price"})

    merged = gmin.merge(tgt, on=["batch_date", "manufacturer", "product_key"], how="left")
...
def should_block_plan_fallback(q: str, slots: Dict[str, Any]) -> bool:
    if slots.get("has_violation"):
        return True
    if slots.get("not_cheapest"):
        return True
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Snacks POC CLI Router v4.2.3")
...
def main():
    ...
    print("=== Snacks POC CLI Router v4.2.3 ===")
    ...
    if args.enable_chroma:
        try:
            ...
            upsert_price_compare(col_sum, df_pc_all, version="v4_2_3")
            if not args.disable_reviews_digest:
                upsert_reviews_digest(col_rev, df_data_all, version="v4_2_3")
            print("Chroma 업서트 완료")
        except Exception as e:
    ...
    while True:
        ...
        if intent == "A1_VIOL_DETAIL":
            ...
            print(f"\n[A급 정형] {manu} '{mall}' 위반 상세 ({title_range}) — 제품 {product_count}개 / 케이스 {case_count}행")
        ...
        if intent == "Q5_RANGE_SUMMARY":
            ...
            print(f"\n[정형결과] {mall}보다 싼 몰별 위반건수 (날짜별) ({start_date} ~ {end_date}{scope})")
        ...
        if intent == "Q5_RANGE_DETAIL":
            ...
            product_count = int(out_df["product_key"].nunique()) if "product_key" in out_df.columns else 0
            case_count = int(len(out_df))
            print(f"\n[정형결과] {mall}보다 싼 곳이 있는 상품 상세 (날짜별) ({start_date} ~ {end_date}{scope}) — 제품 {product_count}개 / 케이스 {case_count}행")
